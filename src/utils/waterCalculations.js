...
/**
 * Normalizē perioda virkni uz YYYY-MM formātu
 * Apstrādā: "2024-3", "2024-03", "2024-03-01", "2024-3-1"
 */
export const normalizePeriod = (p) => {
  if (!p || typeof p !== 'string') return p;
  const parts = p.split('-');
  if (parts.length < 2) return p;
  // Ņemam tikai pirmos divus segmentus (gads un mēnesis), ignorējam dienu
...
export const calculateWaterDetails = ({
  ...
  const getConsumption = (type) => {
    ...
    // ✅ Meklējam ierakstu ar stingru tipu un perioda pārbaudi
    ...
    // ✅ Ja ieraksts eksistē water_consumption tabulā — VIENMĒR izmantojam to,
    // pat ja consumption_m3 === 0 (0 ir derīga vērtība, nevis kļūda)
  ...
  if (hotWaterTariff && hotWaterTariff.include_in_invoice !== false) {
    ...
    const amount = Math.round(m3 * price * 100) / 100;
    const vatRate = (hotWaterTariff.vat_rate !== undefined && hotWaterTariff.vat_rate !== null)
    ...
    if (hotM3 === null && parseFloat(hotWaterTariff.diff_m3 || 0) > 0) {
      const count = nonReportingHotCount ?? safeApts.filter(a => {
        ...
        const hasMr = (meterReadings || []).some(mr =>
          String(mr.apartment_id) === String(a.id) &&
          String(mr.meter_type) === 'hot_water' &&
          normalizePeriod(mr.period) === normPeriod &&
          mr.reading_value !== null);
        return !hasWc && !hasMr;
      }).length;
...
export const calculateWaterGlobalSummary = (period, waterConsumption, waterTariff, hotWaterTariff) => {
  ...
  const getMetrics = (type, tariff) => {
    ...
    const totalM3 = waterConsumption
      .filter(wc => normalizePeriod(wc.period) === normPeriod && String(wc.meter_type) === String(type))
    ...
    return { m3: totalM3, amountWithoutVat, amountWithVat };
  };
