...
export default function PropertyManager() {
  ...
  const [overpaymentModal, setOverpaymentModal] = useState(null);
  ...
  const saveDebtNote = async (invoiceId, debtNote) => {
    ...
    if (!error) {
      fetchInvoices();
      setDebtModal(null);
      showToast('Parāda piezīme saglabāta', 'success');
    } else {
      showToast('Kļūda saglabājot piezīmi', 'error');
    }
  };
  ...
  const saveOverpayment = async (invoiceId, amount, month) => {
    ...
    if (!error) {
      ...
      showToast('Pārmaksa reģistrēta', 'success');
    } else {
      showToast('Kļūda reģistrējot pārmaksu', 'error');
    }
  };
